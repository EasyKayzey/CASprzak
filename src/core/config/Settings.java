...
public class Settings {
	...
	/**
	 * The amount of times that {@link core.tools.singlevariable.Solver} will run unless the exit conditions are met beforehand.
	 */
	public static int defaultSolverIterations = 100;

	/**
	 * The amount of segments which {@link core.tools.singlevariable.Solver} splits a given range into.
	 */
	public static int defaultRangeSections = 29;

	/**
	 * The number of segments that {@link core.tools.singlevariable.NumericalIntegration#simpsonsRule(GeneralFunction, double, double)} uses when performing numerical integration. **MUST BE EVEN**
	 */
	...
	/**
	 * The variable to be used in {@link core.tools.singlevariable} when none is specified.
	 */
	public static String singleVariableDefault = "x";

	/**
	 * The margin that {@link core.tools.singlevariable.Solver} uses to determine if a value is close enough to zero to be considered a root of a function.
	 */
	...
	/**
	 * Forces functions in {@link core.functions.unitary.integer} to return integers when using approximations.
	 */
	public static boolean enforceIntegerOperations = true;

	/**
	 * Denotes whether methods in {@link core.tools.singlevariable} should exit if the result is within a certain proximity of the target. Improves performance at the cost of accuracy.
	 */
	...
	public static void parseSingleSetting(String key, String value) {
		switch (key) {
			...
			case "escapeNames"				 				-> escapeNames = ParsingTools.parseBoolean(value);

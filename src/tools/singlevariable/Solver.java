...
public class Solver {
	...
	public static double getSolutionPointNewton(GeneralFunction expression, double initialPoint, int runs) {
		...
		for (int i = 0; i < runs; i++) {
			...
			if (i % 25 == 0 && initialPoint < 1E-10 && initialPoint > -1E-10)
				return 0;
			if (Settings.exitSolverOnProximity && i % (runs / 20) == 0 && Math.abs(expression.evaluate(Map.of(Settings.singleVariableDefault, nextPoint))) < Settings.equalsMargin)
				return nextPoint;
		}
	...
	public static double[] getSolutionsRangeNewton(GeneralFunction expression, double lower, double upper, int runs) {
		...
		while (iter.hasNext()) {
			...
			if (Math.abs(expression.evaluate(Map.of(Settings.singleVariableDefault, nextLocation))) > Settings.zeroMargin)
				iter.remove();
			else

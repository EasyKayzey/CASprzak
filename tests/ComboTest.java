...
public class ComboTest {
	...
	void basicFactorial2() {
		GeneralFunction test = Parser.parse("3!+2");
		assertEquals(8, test.evaluate(Map.of('x', 3.5)));
	}

	@Test
	void parenFactorial2() {
		GeneralFunction test = Parser.parse("(2*x-54)!+2");
		assertEquals(26, test.evaluate(Map.of('x', 29.0)));
	}

	@Test
	void absFactorial1() {
		GeneralFunction test = Parser.parse("(|x|)!"); // Might want to make this test use precedence when prefix precedence is added
	...
	@Test
	void test6C4() {
		GeneralFunction test = Parser.parseSimplified("6C4");
		assertEquals(15, test.evaluate(null));
	}

	@Test
	void test6P4() {
		GeneralFunction test = Parser.parseSimplified("6P4");
		assertEquals(360, test.evaluate(null));
	}

	@Test
	void test6Cx() {
		GeneralFunction test = Parser.parseSimplified("6Cx");

...
public class IntegralTest {
    ...
    void splitAdds() throws Exception {
        ...
        assertEquals(test1.execute().simplify(settings), test2);
    }
    ...
    void unwrapsExponents() throws Exception {
        ...
        assertEquals(test2, test1.execute().simplify(settings));
    }
    ...
    void simpleOPIsOne() throws Exception {
        GeneralFunction test1 = new Integral(FunctionParser.parseSimplified("\\cos(x)*\\sin(x)"), "x").execute().simplify(settings);
